    ...
    while (!done) {
        ...
        if (!GetLogicalProcessorInformation(buffer, &length))
        {
            ...
            while (byteOffset + sizeof(SYSTEM_LOGICAL_PROCESSOR_INFORMATION) <= length)
            {
                ++count;
                char content[1024];
                sprintf_s(content, "Logical processor found %d\n---Relationship:%s\n---ProcessorMask:%s\n",
                    count,
                    to_string(ptr->Relationship).c_str(),
                    to_string(ptr->ProcessorMask).c_str());
                writeToFile(content, (char*)"C:\\Facultate\\CSSO\\Laboratoare\\Week6\\info.txt");
                cout << content << '\n';
...
void getNUMA()
{
    ...
    if (!GetNumaHighestNodeNumber(&highestNodeNumer))
    {
        ...
        char content[1024];
        strcpy_s(content, "\nHighest node number: ");
        strcat_s(content, to_string(highestNodeNumer).c_str());
        writeToFile(content, (char*)"C:\\Facultate\\CSSO\\Laboratoare\\Week6\\info.txt");
    ...
    if (!GetNumaNodeProcessorMask((UCHAR)highestNodeNumer, &processorMask))
    {
        ...
        strcat_s(content, to_string(processorMask).c_str());
        writeToFile(content, (char*)"C:\\Facultate\\CSSO\\Laboratoare\\Week6\\info.txt");
    }
}
...
void getCPUSets()
{
    ...
    if (!GetSystemCpuSetInformation(infoBuf, returnedLength,
        ...
        strcpy_s(content, "\nNumber of CPU cores: ");
        strcat_s(content, to_string(count).c_str());
        for (size_t i = 0; i < count; i++) {
            cout << "CPU core ID: " << infoBuf[i].CpuSet.Id << '\n';
            strcat_s(content, "\nCPU core ID: ");
            strcat_s(content, to_string(infoBuf[i].CpuSet.Id).c_str());
        }
        writeToFile(content, (char*)"C:\\Facultate\\CSSO\\Laboratoare\\Week6\\info.txt");
    }
