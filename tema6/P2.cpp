...
void writeToFile(char* text, char* file)
{
   ...
   /* h = CreateFile(file, GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);*/
...
void sequentialMul(int** a, int** b, int size, int k, int index)
{
    ...
    char fileName[256];
    sprintf_s(fileName, "C:\\Facultate\\CSSO\\Laboratoare\\Week6\\rezultate\\secvential\\Seq_%d_%d_%s.txt",
...
DWORD WINAPI multiplicateMatrix(LPVOID lpParam)
{
    ...
    duration<double> time_span = duration_cast<duration<double>>(after - before);
    char fileName[256];
    sprintf_s(fileName, "C:\\Facultate\\CSSO\\Laboratoare\\Week6\\rezultate\\static\\%d_%d_%d_%s.txt",
...
{
    char fileName[256];
    sprintf_s(fileName, "C:\\Facultate\\CSSO\\Laboratoare\\Week6\\date\\%d_%d_%c.txt",
        k, matrixIndex, name);
