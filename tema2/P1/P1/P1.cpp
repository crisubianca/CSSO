...
void setPrivilege() {
    ...
    if (OpenProcessToken(hProcess, TOKEN_ADJUST_PRIVILEGES, &tokenHandle)) {
        ...
        if (!LookupPrivilegeValueW(NULL, SE_SYSTEM_PROFILE_NAME, &luid)) {
            cout << "LookupPrivilegeValueW error: " << GetLastError() << endl;
        }
        ...
        if (GetLastError() == ERROR_NOT_ALL_ASSIGNED) {
            cout << "The token doesn't have the specified privilege! error: " << GetLastError() << endl;
        }
...
bool createFile(string fileName) {
    ...
    char finalFile[300] = "";
    strcat_s(finalFile, "C:\\Facultate\\CSSO\\Laboratoare\\Week2\\");
    strcat_s(finalFile, fileName.c_str());

    //create file using handle
    h = CreateFileA(finalFile
        , GENERIC_READ, NULL, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
...
int ModuleInfo() {
    ...
    if (!Module32First(hProcessSnap, &me32)) {
        cout << "Module32First error: " << GetLastError() << endl;
        CloseHandle(hProcessSnap);
    }
...
void writeToMemoryMap(char* message) {
    ...
    wcscpy_s(sz_message, (TCHAR*)message);
    ...
    if (mapHandle == NULL) {
        cout << "OpenFileMapping error: " << GetLastError() << endl;
    }
